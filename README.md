   * [一、预先准备](#一预先准备)
      * [1.1 服务器角色](#11-服务器角色)
      * [1.2 系统初始化](#12-系统初始化)
      * [1.3 hosts设置](#13-hosts设置)
         * [1.3.1 通过hosts文件配置](#131-通过hosts文件配置)
         * [1.3.2 使用bind9 DNS工具](#132-使用bind9-dns工具)
      * [1.4 （可选）安装supervisor](#14-可选安装supervisor)
   * [二、安装CRI容器运行时及harbor私有仓](#二安装cri容器运行时及harbor私有仓)
      * [2.1 安装containerd](#21-安装containerd)
         * [2.1.1 步骤一安装containerd](#211-步骤一安装containerd)
         * [2.1.2 步骤二安装runC](#212-步骤二安装runc)
         * [2.1.3 步骤三安装CNI插件](#213-步骤三安装cni插件)
         * [2.1.4 配置containerd使用systemd cgroup驱动](#214-配置containerd使用systemd-cgroup驱动)
         * [2.1.5 安装crictl客户端工具](#215-安装crictl客户端工具)
      * [2.2 安装harbor私有仓](#22-安装harbor私有仓)
   * [三、使用cfssl生成证书](#三使用cfssl生成证书)
      * [3.1 安装cfssl](#31-安装cfssl)
      * [3.2 初始化及说明](#32-初始化及说明)
   * [四、部署master节点](#四部署master节点)
      * [4.1 部署etcd集群](#41-部署etcd集群)
         * [4.1.1 下载安装etcd和etcdctl二进制文件](#411-下载安装etcd和etcdctl二进制文件)
         * [4.1.2 为etcd制作证书](#412-为etcd制作证书)
         * [4.1.3 配置及启动etcd集群](#413-配置及启动etcd集群)
      * [4.2 部署API server](#42-部署api-server)
         * [4.2.1 使用cfssl生成证书](#421-使用cfssl生成证书)
         * [4.2.2 部署api-server](#422-部署api-server)
      * [4.3 部署controller-manager](#43-部署controller-manager)
      * [4.4 部署kube-scheduler](#44-部署kube-scheduler)
      * [4.5 配置kubectl所需要的kubeconfig](#45-配置kubectl所需要的kubeconfig)
      * [4.6 部署node相关组件kube-proxy、kubelet](#46-部署node相关组件kube-proxykubelet)
         * [4.6.1 bootstrap自动认证kubelet](#461-bootstrap自动认证kubelet)
         * [4.6.2 部署kubelet](#462-部署kubelet)
         * [4.6.3 将master节点作为node加入到集群内部](#463-将master节点作为node加入到集群内部)
         * [4.6.3 部署kube-proxy](#463-部署kube-proxy)
         * [4.6.4 授权apiserver访问kubelet](#464-授权apiserver访问kubelet)
         * [4.6.5 部署flannel网络组件](#465-部署flannel网络组件)
      * [4.7 检查master节点部署结果](#47-检查master节点部署结果)
         * [4.7.1 检查服务运行状态](#471-检查服务运行状态)
         * [4.7.2 附：文件列表及简介](#472-附文件列表及简介)
   * [五、部署node节点](#五部署node节点)
      * [5.1 部署kubelet](#51-部署kubelet)
      * [5.2 部署kube-proxy](#52-部署kube-proxy)
      * [5.3 检查node节点部署结果](#53-检查node节点部署结果)
         * [5.3.1 检查服务运行状态](#531-检查服务运行状态)
         * [5.3.2 附：文件列表及简介](#532-附文件列表及简介)
   * [六、其他可选组件安装](#六其他可选组件安装)
      * [6.1 coredns](#61-coredns)
         * [6.1.1 部署coredns](#611-部署coredns)
         * [测试coredns](#测试coredns)
      * [6.2 nginx ingress](#62-nginx-ingress)
      * [6.3 面板](#63-面板)
         * [6.3.1 dashboard](#631-dashboard)
         * [6.3.2 rancher](#632-rancher)
   * [七、未完待续](#七未完待续)
